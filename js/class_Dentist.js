...
export class VisitDentist extends Visit {
    constructor(id, urgencyOfVisit, doctor, visitStatus, nameDoctor, date, nameUser, purpose, description, age, pressure, bodyIndex, diseases, lastVisit) {
        super(id, urgencyOfVisit, doctor, visitStatus, nameDoctor, date, nameUser, purpose, description);
        this.lastVisit = lastVisit;
    }

    async createDentist(token) {
    ...
    async createDentistForm() {
        ...
        labelLastVisit.textContent = "Date of last visit"
        inputLastVisit.classList.add("last-visit")
        inputLastVisit.name = "lastVisit"
        inputLastVisit.placeholder = "Date of last visit:"
        inputLastVisit.type = "date"
        inputLastVisit.setAttribute("required", "required")
        ...
        form.append(select, inputDoctor, labelDate, inputDate, selectStatus, inputNameDoctor, inputPurpose, inputNameUser, labelLastVisit, inputLastVisit, textarea, btn)
        ...
        document.querySelector(".form__create-information").addEventListener("submit", async (event) => {
            ...
            this.lastVisit = event.target.elements.lastVisit.value
            ...
            const visit = await this.createDentist(`${localStorage.getItem("token")}`)
            this.id = visit.id

            document.querySelector('.section__form').style.display = "none"
            this.renderDentistCard(this)
    ...
    renderDentistCard(item) {
        ...
        const templateCard = `
                          <li class="cards__content cards__${item.id}">
                            ...
                            <img class="cards__img" src="../img/dentist.png" alt="doctor">
                            ...
                            <div class="cards__more-information">
                                <ol class="list-group list-group-numbered card__${item.id}" style="display: none;">
                                    ...
                                    <li  data-id = ${item.id} class="list-group-item"><span>Date of your last visit: </span>${item.lastVisit}</li>
    ...
    render() {
        this.createDentistForm(`${localStorage.getItem("token")}`)
