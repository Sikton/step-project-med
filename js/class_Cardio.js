import {Visit} from "./class_Visit.js"
...
export class VisitCardiologist extends Visit {
    ...
    async createCardCardiologist(token) {
        const cardio = {
    ...
    async createCardioForm() {
        ...
        inputDoctor.value = 'Cardiologist'
        ...
        inputPressure.classList.add("pressure")
        inputPressure.name = "pressure"
        inputPressure.placeholder = "Normal pressure"
        inputPressure.setAttribute("required", "required")

        const inputBodyIndex = document.createElement("input")
        inputBodyIndex.classList.add("bodyIndex")
        inputBodyIndex.name = "bodyIndex"
        inputBodyIndex.placeholder = "Body mass index"
        inputBodyIndex.type = "number"
        inputBodyIndex.setAttribute("required", "required")

        const inputDiseases = document.createElement("input")
        inputDiseases.classList.add("diseases")
        inputDiseases.name = "diseases"
        inputDiseases.placeholder = "A history of cardiovascular disease?"
        inputDiseases.setAttribute("required", "required")
        ...
        form.append(select, inputDoctor, selectStatus, inputNameDoctor, inputPurpose, inputAge, labelDate, inputDate, inputNameUser, inputPressure, inputBodyIndex, inputDiseases, textarea, btn)
        ...
        document.querySelector(".form__create-information").addEventListener("submit", async (event) => {
            ...
            this.pressure = event.target.elements.pressure.value
            this.bodyIndex = event.target.elements.bodyIndex.value
            this.diseases = event.target.elements.diseases.value
            this.description = event.target.elements.description.value

            const visit = await this.createCardCardiologist(`${localStorage.getItem("token")}`)
            this.id = visit.id

            document.querySelector('.section__form').style.display = "none"
            this.renderCardioCard(this)
    ...
    renderCardioCard(item) {
        ...
        const templateCard = `
                          <li class="cards__content cards__${item.id}">
                            ...
                            <img class="cards__img" src="img/cardio.png" alt="doctor">
                            ...
                            <ol class="list-group list-group-numbered card__${item.id}" style="display: none;">
                                ...
                                <li data-id = ${item.id} class="list-group-item"><span>Your normal pressure: </span>${item.pressure}</li>
                                <li data-id = ${item.id} class="list-group-item"><span>Your body mass index: </span>${item.bodyIndex}</li>
                                <li data-id = ${item.id} class="list-group-item"><span>Your history of cardiovascular disease:</span>${item.diseases}</li>
    ...
    render() {
        this.createCardioForm(`${localStorage.getItem("token")}`)
    }
}